...
> Condensed from the spec. See `specs/` for the full reasoning.
...
| **PI** (your agent) | Sessions, transcripts, compaction, tool policies, MCP tools, the agent loop, reasoning | Channel I/O |
...
presentation evolve on their own cadence.
...
is included in the prompt prefix** so PI can call telegramy media tools
directly for audio, photos, and files. Text replies are handled by the relay.
...
Each Telegram chat gets **one persistent session file** (`<project>/sessions/chat_{id}.jsonl`),
...
turn's `abort` escalates to kill. Warmth is the feature — never tear down a
healthy warm process speculatively.

## PI profile

PI has no `--config-dir` flag. A profile is a **project directory** that PI
loads from its cwd: `AGENTS.md` (developer constitution), `.pi/SYSTEM.md`
(system prompt), `.pi/extensions/*.ts` (MCP tools). The relay spawns PI with
`cwd=<project dir>` and `-a` (trust). The default directory name is `hal/`
but you can rename it — just set `RELAYPI_PROJECT_DIR`.

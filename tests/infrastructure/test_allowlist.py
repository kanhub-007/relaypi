...
def _msg(chat_id, user_id, chat_type="private", username="x"):
    return parse_inbound(msg_event(chat_id, username, "hi", chat_type=chat_type, user_id=user_id))
...
def test_restricted_group_allows_only_listed_members():
    allowlist = ConfigAllowlist(
        dm_users=set(),
        groups={-100222000: GroupConfig(mode="restricted", members=frozenset({111222333}))},
    )
    # Member -> allowed
    assert allowlist.allows(_msg(-100222000, user_id=111222333, chat_type="group")) is True
    # Non-member -> dropped
...
def test_unconfigured_group_chat_dropped():
    allowlist = ConfigAllowlist(dm_users={987654321}, groups={})
    assert allowlist.allows(_msg(-999999, user_id=987654321, chat_type="group")) is False


def test_unconfigured_chat_type_dropped():
    allowlist = ConfigAllowlist(dm_users={987654321}, groups={})
    # "channel" is neither private nor a configured group
    assert allowlist.allows(_msg(-100333000, user_id=987654321, chat_type="channel")) is False

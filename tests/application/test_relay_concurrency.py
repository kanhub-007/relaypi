...
async def test_same_chat_messages_are_serialized_in_order():
    ...
    assert pi.commands[0]["message"] == "[from=alice][chat=123] first"
    ...
    assert pi.commands[1]["message"] == "[from=alice][chat=123] second"

    pi.pending[1].set_result(None)
    await drain_task

    assert [c["message"] for c in pi.commands] == [
        "[from=alice][chat=123] first",
        "[from=alice][chat=123] second",
    ]
...
async def test_different_chats_run_concurrently():
    ...
    assert clients["123"].commands[0]["message"] == "[from=alice][chat=123] hello"
    assert clients["456"].commands[0]["message"] == "[from=alice][chat=456] hi"
